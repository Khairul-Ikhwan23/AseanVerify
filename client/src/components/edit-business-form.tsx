...
export default function EditBusinessForm({ business, onClose, onSuccess }: EditBusinessFormProps) {
  ...
  // Fetch user's current secondary affiliates
  ...
  // Memoize the secondary affiliate IDs to prevent infinite re-renders
  const secondaryAffiliateIds = useMemo(() => {
    return userSecondaryAffiliates.map((usa: any) => usa.secondaryAffiliateId);
  }, [userSecondaryAffiliates]);
  const [formData, setFormData] = useState({
    ...
    ownerNames: business.ownerName ? business.ownerName.split(", ").filter(name => name.trim()) : [""], // Split existing owner names
    yearEstablished: business.yearEstablished || "",
    numberOfEmployees: business.numberOfEmployees || "6 - 20",
    primaryAffiliateId: business.primaryAffiliateId || "",
    secondaryAffiliateIds: [] as string[],
    profilePicture: business.profilePicture || "",
    businessLicenseDocuments: Array.isArray(business.businessLicenseDocuments) 
      ? business.businessLicenseDocuments 
      : business.businessLicense ? [business.businessLicense] : [],
    registrationCertificateDocuments: Array.isArray(business.registrationCertificateDocuments) 
      ? business.registrationCertificateDocuments 
      : business.registrationCertificate ? [business.registrationCertificate] : [],
    proofOfOperationsDocuments: Array.isArray(business.proofOfOperationsDocuments) 
      ? business.proofOfOperationsDocuments 
      : business.proofOfOperations ? [business.proofOfOperations] : [],
  });

  // Debug logging for business documents
  console.log('EditBusinessForm: Business data loaded:', {
    businessLicenseDocuments: business.businessLicenseDocuments,
    registrationCertificateDocuments: business.registrationCertificateDocuments,
    proofOfOperationsDocuments: business.proofOfOperationsDocuments,
    businessLicense: business.businessLicense,
    registrationCertificate: business.registrationCertificate,
    proofOfOperations: business.proofOfOperations,
  });

  // Update form data when business prop changes (only on initial load or when business ID changes)
  useEffect(() => {
    setFormData({
      businessName: business.businessName || "",
      businessEmail: business.businessEmail || "",
  ...
  }, [business.id]); // Only depend on business.id, not secondaryAffiliateIds

  // Update secondary affiliate IDs when they change
  useEffect(() => {
    setFormData(prev => ({ ...prev, secondaryAffiliateIds }));
  }, [secondaryAffiliateIds]);
  ...
  const updateBusinessMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      // Also invalidate any business-specific queries
  ...
  const handleInputChange = (field: string, value: string | string[]) => {
    console.log('Edit Business Form: handleInputChange called with field:', field, 'value:', value);
    setFormData(prev => {
      const newData = { ...prev, [field]: value };
      console.log('Edit Business Form: Updated formData:', newData);
      return newData;
    });
    
    // If it's a document field or profile picture, save immediately to database
    if (field.includes('Documents') || field === 'profilePicture') {
      const updateData = { [field]: value };
      console.log('Saving change immediately:', field, value);
      ...
      .catch(error => {
        ...
        setFormData(prev => ({ ...prev, [field]: business[field as keyof typeof business] }));
      });
    }
  };
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex gap-3 pt-4">
              ...
              <Button
                type="submit"
                className="flex-1"
                disabled={updateBusinessMutation.isPending || !formData.businessName.trim()}
              >
